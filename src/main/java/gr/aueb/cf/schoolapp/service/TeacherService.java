...
public class TeacherService implements ITeacherService {
    ...
    @Override
    @PreAuthorize("hasAuthority('INSERT_TESCHER')")
    ...
    public Page<TeacherReadOnlyDTO> getPaginatedTeachersDeletedFalse(Pageable pageable) {
        ...
        log.debug("Get paginated returned not successfully page={} and size={}",
                teachersPage.getNumber(), teachersPage.getSize());
        return teachersPage.map(mapper::mapToTeacherReadOnlyDTO);
    }
    ...
    public TeacherReadOnlyDTO updateTeacher(TeacherEditDTO dto) throws EntityNotFoundException, EntityAlreadyExistsException, EntityInvalidArgumentException {
        try {
            Teacher teacher = teacherRepository.findByUuid(dto.uuid())
                    .orElseThrow(() -> new EntityNotFoundException(
                            "Teacher with uuid=" + dto.uuid() + "not found."));
            ...
            if (Objects.equals(dto.regionId(), teacher.getRegion())) {
                Region region = regionRepository.findById(dto.regionId())
                        .orElseThrow(() -> new EntityInvalidArgumentException("Region id=" + dto.regionId() + "invalid"));
...
//            automatic rollback
    ...
    public TeacherEditDTO getTeacherByUUIDDeletedFalse(UUID uuid) throws EntityNotFoundException {
        try {
            ...
            log.error("Get non deleted teacher by uuid={} failed.", uuid, e);
            throw e;
        }
    }
}
